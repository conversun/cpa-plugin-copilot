...
import (
	...
	copilotpkg "github.com/conversun/cpa-plugin-copilot/copilot"
	"github.com/conversun/cpa-plugin-copilot/internal/registry"
	"github.com/conversun/cpa-plugin-copilot/internal/helps"
	"github.com/conversun/cpa-plugin-copilot/internal/thinking"
...
func NewGitHubCopilotExecutor() *GitHubCopilotExecutor {
...
func (e *GitHubCopilotExecutor) HttpRequest(ctx context.Context, auth *cliproxyauth.Auth, req *http.Request) (*http.Response, error) {
	...
	httpClient := newProxyAwareHTTPClient(ctx, nil, auth, 0)
...
func (e *GitHubCopilotExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	body = applyPayloadConfigWithRoot(nil, req.Model, to.String(), "", body, originalTranslated, requestedModel, requestPath)
	...
	recordAPIResponseMetadata(ctx, nil, httpResp.StatusCode, httpResp.Header.Clone())

	if !isHTTPSuccess(httpResp.StatusCode) {
		data, _ := io.ReadAll(httpResp.Body)
		appendAPIResponseChunk(ctx, nil, data)
...
func (e *GitHubCopilotExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	if !isHTTPSuccess(httpResp.StatusCode) {
		...
		if readErr != nil {
			recordAPIResponseError(ctx, nil, readErr)
			return nil, readErr
		}
		appendAPIResponseChunk(ctx, nil, data)
		log.Debugf("github-copilot executor: upstream error status: %d, body: %s", httpResp.StatusCode, summarizeErrorBody(httpResp.Header.Get("Content-Type"), data))
	...
	go func() {
		...
		for scanner.Scan() {
			line := scanner.Bytes()
			appendAPIResponseChunk(ctx, nil, line)
		...
		if errScan := scanner.Err(); errScan != nil {
			recordAPIResponseError(ctx, nil, errScan)
...
func (e *GitHubCopilotExecutor) Refresh(ctx context.Context, auth *cliproxyauth.Auth) (*cliproxyauth.Auth, error) {
	...
	copilotAuth := copilotpkg.NewCopilotAuth(nil)
